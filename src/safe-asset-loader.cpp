...
void SafeAssetLoader::loadTexture(sf::Texture& texture, const std::string& filename) {
  ...
  if (!texture.loadFromFile(fullPath)) {
    throw AssetLoadException(AssetLoadException::TEXTURE, filename,
...
void SafeAssetLoader::loadFont(sf::Font& font, const std::string& filename,
  ...
  if (!font.openFromFile(fullPath)) {
    if (!fallbackFont.empty()) {
      ...
      if (font.openFromFile(fallbackPath)) {
        ...
        ErrorHandler::getInstance().logError(
            AssetLoadException(AssetLoadException::FONT, filename,
    ...
    if (loadFallbackFont(font)) {
      ErrorHandler::getInstance().logError(AssetLoadException(
          AssetLoadException::FONT, filename, "Primary font failed, using system default font"));
      return;
    }

    throw AssetLoadException(AssetLoadException::FONT, filename,
...
void SafeAssetLoader::loadSound(sf::SoundBuffer& buffer, const std::string& filename) {
  ...
  if (!buffer.loadFromFile(fullPath)) {
    throw AssetLoadException(AssetLoadException::SOUND, filename,
...
void SafeAssetLoader::createFallbackTexture(sf::Texture& texture, sf::Color color,
                                            sf::Vector2u size) {
  sf::Image fallbackImage(size, color);

  if (!texture.loadFromImage(fallbackImage)) {
    throw SystemResourceException(SystemResourceException::MEMORY,
