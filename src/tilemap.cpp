...
sf::IntRect transPosIntoRect(int pos) {
  ...
  sf::IntRect posRect({(int)cords.x * DP::TILE_SIZE, (int)cords.y * DP::TILE_SIZE},
                      {DP::TILE_SIZE, DP::TILE_SIZE});
  //    std::cout << "debug transPosIntoRect " << pos << " "<<  cords.x << " " << cords.y << " " <<
  //    posRect.position.x << " " << posRect.position.y << std::endl;
  return posRect;
}
...
std::set<int> getNeighbours(int pos) {
  ...
  if (cords.x > 0) {
    ...
    value = pos - 1;
    if ((value != -1) && (terrain.count(value) == 0)) neighboursSet.insert(value);
  }
...
// bool TileMap::load(TextureHolder *textures, sf::Vector2u tileSize, const int* tiles, unsigned int
