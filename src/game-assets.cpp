...
GameAssets::GameAssets(Game* gameInstance) : game(gameInstance) {}
...
void GameAssets::initializeSprites() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    game->playersSprites[i]->setTextureRect(sf::IntRect(
        {game->playersSpritesCords[i][0], game->playersSpritesCords[i][1]}, {280, 280}));
    game->playersSprites[i]->setPosition(
        sf::Vector2f(game->playersSpritesCords[i][0], game->playersSpritesCords[i][1]));
  }
}
