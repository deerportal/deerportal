...
RoundDice::RoundDice(Player (&players)[4]) : sfxDice(sfxDiceBuffer) {
  ...
  diceResultSix = 6;
  diceSize = 150;
  if (!sfxDiceBuffer.loadFromFile(get_full_path(ASSETS_PATH "audio/dice.ogg"))) std::exit(1);

  if (!textureDice.loadFromFile(get_full_path(ASSETS_PATH "img/diceWhite.png"))) std::exit(1);
