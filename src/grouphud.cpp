...
void GroupHud::setFont(sf::Font* gameFont) {
  if (!gameFont) {
    std::cerr
        << "Error: GroupHud::setFont called with null gameFont. Text objects will not be created."
        << std::endl;
  ...
  if (!seasonName)
    seasonName = std::make_unique<sf::Text>(*gameFont);
  else
    seasonName->setFont(*gameFont);

  if (!roundName)
    roundName = std::make_unique<sf::Text>(*gameFont);
  else
    roundName->setFont(*gameFont);

  if (!monthName)
    monthName = std::make_unique<sf::Text>(*gameFont);
  else
