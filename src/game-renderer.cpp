...
void GameRenderer::render(float deltaTime)
{
    ...
    if (game->banner.active)
        game->renderTexture.draw(game->banner);
...
void GameRenderer::renderStateGame()
{
    ...
    if (game->bigDiamondActive)
        game->renderTexture.draw(*game->spriteBigDiamond);
}
    ...
    if (game->currentState == Game::state_game)
    {
        ...
        if (currentMovements[1] > -1)
        {
            if (game->nextRotateElem.active)
                game->renderTexture.draw(game->nextRotateElem);
        }
        
        if (currentMovements[0] > -1)
        {
            if (game->prevRotateElem.active)
                game->renderTexture.draw(game->prevRotateElem);
        }
    }
...
void GameRenderer::setTxtEndGameAmount()
{
    ...
    std::array<ResultTable, 4> results = {
        {
            ResultTable(0, game->players[0].cash, game->players[0].reachedPortal, game->players[0].reachedPortalFirst),
            ResultTable(1, game->players[1].cash, game->players[1].reachedPortal, game->players[1].reachedPortalFirst),
            ResultTable(2, game->players[2].cash, game->players[2].reachedPortal, game->players[2].reachedPortalFirst),
            ResultTable(3, game->players[3].cash, game->players[3].reachedPortal, game->players[3].reachedPortalFirst)
        }
    };
