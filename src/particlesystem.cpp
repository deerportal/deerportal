...
ParticleSystem::ParticleSystem(int width, int height) : m_transparent(sf::Color::Transparent) {
  m_image.resize(sf::Vector2u(static_cast<unsigned int>(width), static_cast<unsigned int>(height)),
                 m_transparent);
...
void ParticleSystem::fuel(int particles) {
  for (int i = 0; i < particles; ++i) {
    ...
    p->color = sf::Color(m_randomizer.rnd(0, 255), m_randomizer.rnd(0, 255),
                         m_randomizer.rnd(0, 255), 255);
...
void ParticleSystem::update() {
  ...
  for (ParticleIterator it = m_particles.begin(); it != m_particles.end();) {
    ...
    if (p->color.a == 0 || p->pos.x >= static_cast<float>(imgWidth) || p->pos.x < 0 ||
...
void ParticleSystem::render() {
  for (ParticleIterator it = m_particles.begin(); it != m_particles.end(); ++it) {
    Particle* p = *it;
    sf::Vector2u coords(static_cast<unsigned int>(p->pos.x), static_cast<unsigned int>(p->pos.y));
