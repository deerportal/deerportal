...
SoundFX::SoundFX()
  ...
  if (!soundCollectBuffer.loadFromFile(get_full_path(ASSETS_PATH "audio/collect.ogg"))) exit(-1);
  if (!soundCardBuffer.loadFromFile(get_full_path(ASSETS_PATH "audio/card.ogg"))) exit(-1);
  if (!bufferDeerMode.loadFromFile(get_full_path(ASSETS_PATH "audio/dp-deermode.ogg"))) exit(-1);
  if (!bufferMeditation.loadFromFile(get_full_path(ASSETS_PATH "audio/dp-meditation.ogg")))
    exit(-1);
  if (!bufferPortal.loadFromFile(get_full_path(ASSETS_PATH "audio/dp-ok.ogg"))) exit(-1);
