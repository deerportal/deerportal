...
TextureHolder::TextureHolder()
{
    ...
    for (int i=0;i<4;i++)
    {
        for (int j=0;j<4;j++)
        {
            if(!cardsTextures[i][j].loadFromFile(get_full_path("assets/img/cards/"+cardsImages[i][j])))
            {
...
//                std::cout << "assets/img/cards/" << cardsImages[i][j] << " ok " << i << " " <<  j << " " << std::endl;
    ...
    if (!textureCardBases[0].loadFromFile(get_full_path("assets/img/card-water-2-diam_m.png")))
        std::exit(1);

    if (!textureCardBases[1].loadFromFile(get_full_path("assets/img/card-earth-2-diam_m.png")))
        std::exit(1);

    if (!textureCardBases[2].loadFromFile(get_full_path("assets/img/card-fire-2-diam_m.png")))
        std::exit(1);

    if (!textureCardBases[3].loadFromFile(get_full_path("assets/img/card-air-2-diam_m.png")))
        std::exit(1);
...
//        std::exit(1);
    ...
    if (!textureCharacters.loadFromFile(get_full_path("assets/img/characters-new.png")))
        std::exit(1);

    if (!backgroundDark.loadFromFile(get_full_path("assets/img/background_dark.png")))
        std::exit(1);
    if (!textureBoardDiamond.loadFromFile(get_full_path("assets/img/board_diamonds.png")))
        std::exit(1);

    if (!textureMenu.loadFromFile(get_full_path("assets/img/dp_intro_menu.png")))
        std::exit(1);

    if (!textureLetsBegin.loadFromFile(get_full_path("assets/img/letsbegin.png")))
        std::exit(1);


    if (!textureButtonCpu.loadFromFile(get_full_path("assets/img/button-cpu.png")))
        std::exit(1);

    if (!textureButtonHuman.loadFromFile(get_full_path("assets/img/button-human.png")))
        std::exit(1);

    if (!textureDeerGod.loadFromFile(get_full_path("assets/img/deer-god.png")))
        std::exit(1);

    if (!textureBigDiamond.loadFromFile(get_full_path("assets/img/diamond-big.png")))
        std::exit(1);
