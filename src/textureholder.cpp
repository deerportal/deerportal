...
TextureHolder::TextureHolder() {
  ...
  if (!textureMenu.loadFromFile(get_full_path(ASSETS_PATH "img/dp_intro_menu.png"))) std::exit(1);

  if (!textureLetsBegin.loadFromFile(get_full_path(ASSETS_PATH "img/letsbegin.png"))) std::exit(1);

  if (!textureButtonCpu.loadFromFile(get_full_path(ASSETS_PATH "img/button-cpu.png"))) std::exit(1);

  if (!textureButtonHuman.loadFromFile(get_full_path(ASSETS_PATH "img/button-human.png")))
    std::exit(1);

  if (!textureDeerGod.loadFromFile(get_full_path(ASSETS_PATH "img/deer-god.png"))) std::exit(1);
