...
sf::Vector2f AnimatedBoardItem::calculateBezierPosition(float t) const {
  ...
  return invT3 * bezierPoints[0] + 
         3.0f * invT2 * t * bezierPoints[1] + 
         3.0f * invT * t2 * bezierPoints[2] + 
         t3 * bezierPoints[3];
}
...
void AnimatedBoardItem::addToVertexArray(sf::VertexArray& vertices, int vertexOffset, const sf::Texture& texture) const {
  ...
  for (int i = 0; i < 4; i++) {
    rotatedCorners[i] = sf::Vector2f(
      corners[i].x * cosAngle - corners[i].y * sinAngle,
