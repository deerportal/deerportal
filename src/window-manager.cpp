...
void WindowManager::initializeVideoModes() {
  ...
  std::cout << "  Initial Fullscreen: " << m_fullscreenMode.size.x << "x" << m_fullscreenMode.size.y
            << std::endl;
#endif
}

bool WindowManager::toggleFullscreen(sf::RenderWindow& window, sf::RenderTexture& renderTexture,
...
#ifndef NDEBUG
      std::cout << "Detected target monitor: " << targetFullscreenMode.size.x << "x"
                << targetFullscreenMode.size.y << std::endl;
#endif
...
void WindowManager::updateView(sf::RenderTexture& renderTexture) {
  ...
  m_view.setViewport(sf::FloatRect(sf::Vector2f(viewportX, viewportY),
  ...
  std::cout << "  Game: " << gameWidth << "x" << gameHeight << " (aspect: " << gameAspectRatio
            << ")" << std::endl;
  std::cout << "  Viewport: " << viewportX << ", " << viewportY << ", " << viewportWidth << ", "
...
void WindowManager::updateRenderTextureSize(sf::RenderTexture& renderTexture,
...
#ifndef NDEBUG
  std::cout << "Render texture maintained at native resolution: " << gameResolution.x << "x"
            << gameResolution.y << std::endl;
#endif
}
...
sf::VideoMode WindowManager::detectCurrentMonitorMode(sf::RenderWindow& window) {
  ...
  sf::Vector2i windowCenter(windowPos.x + static_cast<int>(windowSize.x) / 2,
  ...
  if (windowCenter.x >= static_cast<int>(primaryMode.size.x) ||
      windowCenter.y >= static_cast<int>(primaryMode.size.y) || windowCenter.x < 0 ||
      windowCenter.y < 0) {
...
#ifndef NDEBUG
        std::cout << "Window detected on secondary monitor, using mode: " << mode.size.x << "x"
                  << mode.size.y << std::endl;
#endif
        return mode;
      }
    }
...
#ifndef NDEBUG
  std::cout << "Using primary monitor mode: " << primaryMode.size.x << "x" << primaryMode.size.y
            << std::endl;
#endif
  return primaryMode;
}
